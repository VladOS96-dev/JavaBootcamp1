...
public class UsersRepositoryImpl implements UsersRepository {
    ...
    public Optional<User> findByUsername(String username) {
        return jdbcTemplate.query("SELECT * FROM app.users WHERE username = ?", userRowMapper, username)
                .stream().findFirst();
    }

    @Override
    public User findById(Long id) {
        List<User> users = jdbcTemplate.query("SELECT * FROM app.users WHERE id = ?", userRowMapper, id);
    ...
    public void save(User user) {
        jdbcTemplate.update("INSERT INTO app.users (username, password) VALUES (?, ?)",
                user.getUsername(), user.getPassword());
    }

    @Override
    public void update(User user) {
        jdbcTemplate.update("UPDATE app.users SET username = ?, password = ? WHERE id = ?",
                user.getUsername(), user.getPassword(), user.getId());
    }

    @Override
    public void delete(Long id) {
        jdbcTemplate.update("DELETE FROM app.users WHERE id = ?", id);
    }
}