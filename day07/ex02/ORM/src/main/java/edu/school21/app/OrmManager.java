...
public class OrmManager {
                    ...
                    for (Field f : aClass.getDeclaredFields()) {
                        if (f.isAnnotationPresent(OrmColumnId.class)) {
                            ...
                            sql_create_table += "\t" + column.name() + " " +
                                    sqlType(f.getType().getSimpleName(), 20, aClass.getName() + "." + f.getName()) + ",\n";
                        }
                    }
                    sql_create_table += ");";
    ...
    private String sqlType(String typeName, int length, String classWithField) {
        if (typeName.equals("Integer")) { return "INT"; }
            ...
            return "VARCHAR(" + length + ")"; // Use the 'length' parameter from annotation
        }
    ...
    public void save(Object object) {
        ...
        if (aClass.isAnnotationPresent(OrmEntity.class)) {
            try {
                ...
                for (Field f : aClass.getDeclaredFields()) {
                    ...
                    if (f.isAnnotationPresent(OrmColumnId.class)) {
                        ...
                        if (f.get(object) != null) {
                            ...
                            sql_values += "NULL, "; // Если значение null, используйте SQL NULL
                        }
                    }
                }
                ...
                statement.execute();
                System.out.println("SQL query executed: " + sql_save);
            } catch (SQLIntegrityConstraintViolationException e) {
                System.out.println("The id field must be unique! The record was not saved!");
