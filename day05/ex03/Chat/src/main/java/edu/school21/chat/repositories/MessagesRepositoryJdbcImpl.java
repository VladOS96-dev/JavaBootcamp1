...
public class MessagesRepositoryJdbcImpl implements MessagesRepository {
    ...
    public void update(Message message) {
        ...
        try (Connection conn = dataSource.getConnection();
             PreparedStatement stmt = conn.prepareStatement(validateAuthorQuery)) {
            stmt.setLong(1, message.getAuthor().getUserID());
            try (ResultSet rs = stmt.executeQuery()) {
                if (!rs.next() ||
            ...
            throw new RuntimeException("Ошибка при проверке автора", e);
        }


        String validateRoomQuery = "SELECT id, room_name, owner FROM chat.chatroom WHERE id = ?";
        try (Connection conn = dataSource.getConnection();
             PreparedStatement stmt = conn.prepareStatement(validateRoomQuery)) {
            stmt.setLong(1, message.getRoom().getId());
            try (ResultSet rs = stmt.executeQuery()) {
                if (!rs.next() ||
                        !rs.getString("room_name").equals(message.getRoom().getNameRoom()) ||
                        rs.getLong("owner") != message.getRoom().getOwner().getUserID()) {
                    throw new NotSavedSubEntityException("Комната не существует или данные владельца комнаты не совпадают");
                }
            }
        } catch (SQLException e) {
            throw new RuntimeException("Ошибка при проверке комнаты", e);
        }
